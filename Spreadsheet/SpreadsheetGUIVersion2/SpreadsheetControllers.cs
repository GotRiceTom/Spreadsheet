      ...
      public SpreadsheetControllers(SpreadsheetView ViewInput)
        {
            ...
            //Make a valid regex for spreadsheet
            Regex newVaildRegex = new Regex(@"^[a-zA-Z]?[1-9]?[0-9]?$");

            MainSpreadsheet = new Spreadsheet(newVaildRegex);

            //add each events to their respectfully handler methods
